...
### Fixed
  ...
  * Silence some compiler warnings for generated code about incompatible pointer casts for:
    * Generator scope destruction when calling `_Alta_object_destroy`
    * Casting `_Alta_self` to `_Alta_basic_class` in generator initializers
    * Casting from `_Alta_basic_class` to the appropriate class type in generator executions
    * Using `_Alta_object_destroy` when deleting a variable
  * Properly dereference reference types generated by `to` casts
  * Properly reference reference types when passing them into operator methods
  ...
  * Register symbols for `from` and `to` casts and operator methods
